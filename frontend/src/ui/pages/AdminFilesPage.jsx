...
import { Box, Paper, TextField, MenuItem, Button, TableContainer, Table, TableHead, TableRow, TableCell, TableSortLabel, TableBody, Typography, IconButton, Chip, Menu, ListItemIcon as MListItemIcon } from '@mui/material';
...
export default function AdminFilesPage() {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            a.href = url; a.download = menuFile.originalName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
        } catch { }
        closeMenu();
    };
    const handleApprove = async () => { if (menuFile) { await http.post(`/api/admin/files/${menuFile.id}/status`, { action: 'approve' }); await fetchAdminFiles(); closeMenu(); } };
    const handleDisapprove = async () => { if (menuFile) { await http.post(`/api/admin/files/${menuFile.id}/status`, { action: 'disapprove' }); await fetchAdminFiles(); closeMenu(); } };
    const handleRevert = async () => { if (menuFile) { await http.post(`/api/admin/files/${menuFile.id}/status`, { action: 'revert' }); await fetchAdminFiles(); closeMenu(); } };
    const handleSort = (col) => { if (orderBy === col) setOrder((o) => (o === 'asc' ? 'desc' : 'asc')); else { setOrderBy(col); setOrder('asc'); } };
        ...
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Paper elevation={0} sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: { xs: 1.5, md: 2 }, bgcolor: '#fff', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 1.5 }}>
                    <TextField size="small" label="Status" select value={filters.status} onChange={(e) => setFilters(f => ({ ...f, status: e.target.value }))} sx={{ minWidth: 120 }}>
